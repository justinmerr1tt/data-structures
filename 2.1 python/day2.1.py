...
str = """Game 1: 1 green, 4 blue; 1 blue, 2 green, 1 red; 1 red, 1 green, 2 blue; 1 green, 1 red; 1 green; 1 green, 1 blue, 1 red
...
pattern = re.compile(r'\b(\d+)\s*(blue|red|green)\b')
all_games = []
for line in lines_in:
    matches = pattern.findall(line)
    # convert each match to tuple (number, color)
    tuples = [(int(num), color) for num, color in matches]
    all_games.append(tuples)

# all_games now contains desired structure

for game_index, game in enumerate(all_games):
    check = 0
    # iterate through each number color pair
